...
public class DeleteBookingTest extends ApiBaseTest {
    ...
    private BookingPayloadDTO basePayload() {
        return new PostBookingPayloadFactory().createBookingPayloadDTO(data.bookingFirstname(), data.bookingLastname(), data.bookingTotalprice(), data.bookingDepositpaid(), data.bookingCheckin().toLocalDate(), data.bookingCheckout().toLocalDate(), data.bookingAdditionalneeds());}

    @Test(description = "Delete booking successfully (expects 201 per API behavior)")
    @Severity(SeverityLevel.CRITICAL)
    public void deleteBookingTest() {
        CreateBookingResponseDTO created = postSteps.createBookingWithDTO(basePayload());
